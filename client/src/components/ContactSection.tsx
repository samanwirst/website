...
import type { ContactTexts } from '@/i18n/siteI18n';
...
export function ContactSection({ style, texts }: ContactSectionProps) {
  return (
    <section
      id='contact'
    ...
    >
      <div>
        ...
        <h2 className='m-[0.45rem_0_0] max-w-[18ch] text-[clamp(1.4rem,2.8vw,2.4rem)] font-semibold leading-[1.09] tracking-[-0.03em] [font-family:var(--font-display),Trebuchet_MS,sans-serif]'>
          {texts.titlePrefix} <span className='text-[var(--brand)]'>{texts.titleAccent}</span>
        </h2>
        <p className='m-[0.8rem_0_0] max-w-[55ch] leading-[1.52] text-[var(--section-muted,var(--muted))]'>{texts.summary}</p>
      </div>
